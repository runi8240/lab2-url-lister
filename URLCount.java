...
public class URLCount {
  ...
  public static class URLMapper
    ...
    public void map(Object key, Text value, Context context
      ...
      while (matcher.find()) {
        String url = matcher.group();          // full match e.g. href="https://..."
        if (url.length() > 6) {                // make sure string is long enough
            url = url.substring(6);            // remove first 5 characters
        }
        urlText.set(url);
        context.write(urlText, one);           // emit (url, 1)
      }
    }
  }
